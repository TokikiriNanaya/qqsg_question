...
class ScreenshotTool:
    ...
    def find_correct_answer(self, question_text, options):
        ...
        Returns:
            tuple: (正确选项, 相似度, 选项相似度字典) 或 (None, 0.0, {})
                - 正确选项: 'a', 'b', 'c', 'd' 或 None
                - 相似度: 0.0-1.0（题干匹配度）
        ...
        for q in self.question_bank:
            ...
            if clean_question in bank_question or bank_question in clean_question:
                ...
                return correct_option, 1.0, option_similarities
            else:
                # 否则计算字符集相似度
        ...
        # 如果找到最佳匹配，对比答案
        if best_match and best_similarity > 0.3:  # 相似度阈值设为0.3
            ...
            return correct_option, best_similarity, option_similarities
        
        # 本地题库未匹配，尝试查询网络API
        self.add_log("⚠ 本地题库未找到匹配，正在查询网络题库...")
        online_correct_option, online_option_similarities = self.query_online_api(question_text, options)
        
        if online_correct_option:
            # 网络API找到答案，返回结果（题干相似度设为1.0表示来自网络）
            return online_correct_option, 1.0, online_option_similarities
    ...
    def calculate_option_similarities(self, correct_answer, options):
        ...
        Returns:
            dict: 每个选项的相似度 {'a': 0.85, 'b': 0.3, 'c': 0.2, 'd': 0.1}
        """
        ...
        for option_key, option_text in options.items():
            ...
            if clean_answer in clean_option or clean_option in clean_answer:
                # 包含关系视为高相似度
                option_similarities[option_key] = 1.0
            else:
                # 否则计算字符集相似度
                option_similarities[option_key] = self.similarity_score(clean_answer, clean_option)
    ...
    def update_result_display(self, question_text, answer_texts, correct_option=None, match_similarity=0.0, option_similarities=None):
        """
        ...
        try:
            ...
            for option in ['a', 'b', 'c', 'd']:
                if option in answer_texts and answer_texts[option]:
                    ...
                    # 计算匹配置信度（使用选项相似度）
                    confidence = self.calculate_match_confidence(option, answer_texts, correct_option, match_similarity, option_similarities)
    ...
    def calculate_match_confidence(self, option, answer_texts, correct_option, match_similarity=0.0, option_similarities=None):
        """
        计算选项与题库答案的匹配置信度
    ...
    def setup_gui(self):
        ...
        tools_menu.add_command(label="手动录入", command=self.open_add_question_dialog)
        ...
        ttk.Label(best_answer_frame, text="推荐答案:", font=("Microsoft YaHei", 10, "bold"), width=10).pack(side=tk.LEFT)
    ...
    def perform_ocr_on_areas(self, screenshot, position_info):
        ...
        try:
            ...
            if question_text and answer_texts:
                correct_option, match_similarity, option_similarities = self.find_correct_answer(question_text, answer_texts)
                if correct_option:
                    if match_similarity >= 1.0:
                        self.add_log(f"🎯 正确答案: 选项{correct_option.upper()} ({answer_texts[correct_option]}) [完全匹配]")
                    else:
                        self.add_log(f"🎯 正确答案: 选项{correct_option.upper()} ({answer_texts[correct_option]}) [相似度: {match_similarity:.0%}]")
                else:
                    self.add_log("⚠ 未在题库中找到匹配的题目")
            
            # 更新UI显示区域
            self.update_result_display(question_text, answer_texts, correct_option, match_similarity, option_similarities)
    ...
    def open_add_question_dialog(self, prefill_question=None):
        """打开录入题目对话框
        ...
        dialog = tk.Toplevel(self.root)
        dialog.title("录入新题目")
        dialog.geometry("500x330")  # 减小窗口尺寸
        dialog.resizable(True, True)
        
        # 居中显示
        self.center_window(dialog, 500, 330)
        ...
        # 如果提供了预填充内容则使用，否则检查当前识别结果是否为有效内容
    ...
    def search_online_questions(self, keyword, results_frame, dialog):
        ...
        try:
            ...
            if len(response.text) == 0:
                ttk.Label(results_frame, text="网络API返回空响应", font=DEFAULT_FONT, foreground="#999999").pack(pady=10)
                return
            ...
            if response.status_code == 200:
                ...
                if result.get('success') and result.get('code') == 200:
                    ...
                    if not data:
                        ttk.Label(results_frame, text="网络题库未找到匹配", font=DEFAULT_FONT, foreground="#999999").pack(pady=10)
                        return
                    ...
                    ttk.Label(results_frame, text=f"API返回错误: {result.get('message', '未知错误')}", font=DEFAULT_FONT, foreground="#f44336").pack(pady=10)
            else:
                ttk.Label(results_frame, text=f"请求失败: HTTP {response.status_code}", font=DEFAULT_FONT, foreground="#f44336").pack(pady=10)
        
        except requests.exceptions.Timeout:
            ttk.Label(results_frame, text="网络请求超时", font=DEFAULT_FONT, foreground="#f44336").pack(pady=10)
        except requests.exceptions.JSONDecodeError as e:
            ttk.Label(results_frame, text=f"JSON解析失败: API返回格式错误", font=DEFAULT_FONT, foreground="#f44336").pack(pady=10)
        except Exception as e:
